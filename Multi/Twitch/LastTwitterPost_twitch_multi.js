...
// Multi - Tweet specifics
// [0] PreviousTweetBody / always blank
// [1] CurrentTweetBody / always blank
// [2] PersonNameToPublish = "Trump"; // How to announce this account in chat, can be anything
// [3] TwitterAccountName = "realdonaldtrump"; // name of the Twitter account: https://twitter.com/realdonaldtrump
// [4] TweetOrRetweet = "Tweet" or "Retweet"
var ArrayOfTweeters = [
	{
		PreviousTweetBody: " ",
		CurrentTweetBody: " ",
		PersonNameToPublish: "Trump",
		TwitterAccountName: "realdonaldtrump",
		TweetOrRetweet: "Tweet"
	},
	{
		PreviousTweetBody: " ",
		CurrentTweetBody: " ",
		PersonNameToPublish: "Crowder",
		TwitterAccountName: "scrowder",
		TweetOrRetweet: "Tweet"
	},
	{
		PreviousTweetBody: " ",
		CurrentTweetBody: " ",
		PersonNameToPublish: "Meta",
		TwitterAccountName: "metacritic",
		TweetOrRetweet: "Tweet"
	}
];

// Interval to check for new Tweets in ms (60000 = 1 minute)
// This is per account, we only do 1 at at a time.
// So 3 accounts over 60 seconds would need to be 20000 to update each every 60 seconds
// ...we could also automate this with: TweetCheckTime / ArrayOfTweeters.length
const TweetCheckTimeTotal = 60000;
const TweetCheckTime = TweetCheckTimeTotal / ArrayOfTweeters.length;

// Loop through this ever ActualTweetCheckTime to scrape different accounts
var TweetRotation = 0;
...
function CheckIfRetweet(HTMLData) {
	...
	{
		ArrayOfTweeters[TweetRotation]["TweetOrRetweet"] = "Retweet";
	}
	else
	{
		ArrayOfTweeters[TweetRotation]["TweetOrRetweet"] = "Tweet";
	}
}
...
function ReadLatestPost() {
	...
	request(
		{ uri: "https://mobile.twitter.com/" + ArrayOfTweeters[TweetRotation]["TwitterAccountName"] },
...
// Updates details in multi tweet specific array
function UpdateItemInArray ( value, desc ) {
...
function ProcessLatestPost(body) {
	...
	else
	{
		...
		ArrayOfTweeters[TweetRotation]["CurrentTweetBody"] = "\"" + s_FinalText + "\"";

		// This builds our complete final message
		s_FinalText = (ArrayOfTweeters[TweetRotation]["PersonNameToPublish"] + ' ' + ArrayOfTweeters[TweetRotation]["TweetOrRetweet"] + ': [' + TweetTime + '] ' + s_FinalText);
		...
		if (DuplicateTweets == true)
		{
			...
			ArrayOfTweeters[TweetRotation]["PreviousTweetBody"] = ArrayOfTweeters[TweetRotation]["CurrentTweetBody"];
		}
		else
		{
			// Only post to chat if it's a new message
			// Same commands as above 
			if (ArrayOfTweeters[TweetRotation]["PreviousTweetBody"] !== ArrayOfTweeters[TweetRotation]["CurrentTweetBody"]) {
				client.say(opts.channels[0], "/me " + s_FinalText);
				console.log(s_FinalText);
				ArrayOfTweeters[TweetRotation]["PreviousTweetBody"] = ArrayOfTweeters[TweetRotation]["CurrentTweetBody"];
			}
	...
	// Go to next Tweeter in the list
	TweetRotation += 1;
	
	// If we're past teh last Tweeter, go back to first
	if (TweetRotation >= ArrayOfTweeters.length)
	{
		TweetRotation = 0;
	}

	// Scrape the page again after this amount of time...
	setTimeout(ReadLatestPost, TweetCheckTime); // 1 minute (60000)
}
